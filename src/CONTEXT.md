...
- **Audit remediation (Phase 1–2):** the full middleware audit is tracked in `../docs/plan-remediation.md` (5 phases, 17 tickets). Phase 1 (Safety Net: `ErrorBoundary`, hardened `formatRelativeTime`, `useStore` selector) and Phase 2 (Mutation Integrity) are complete: a shared `useMutation` hook lives at `lib/useMutation.ts`; the chat surface gates send/inject by `role === 'operator'`; lifecycle consolidation uses `ConfirmMutationDialog` + `useMutation`; session pause/resume/complete route through `useMutation` with `ErrorState` surfacing the real kernel error; memory/plan reason fallbacks removed so an empty reason is never faked (buttons disable instead).
