...
- **Invoke commands** (`@tauri-apps/api/core` `invoke`): `op_login(endpoint, username, password) → role`, `op_get_state() → StateOfRecord` (hydrate on mount), `op_create_session`, `op_send_message`, `op_inject_correction`, `op_pause_session`, `op_resume_session`, `op_resolve_hitl`, `op_set_tool_grant`, `op_set_scope`, `op_register_mcp`, `op_register_skill`, `op_trigger_consolidation`, `op_list_tools`, `op_list_skills`. All async; errors come back as strings — surface the real message, not a generic toast.
