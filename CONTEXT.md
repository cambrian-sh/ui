...
| Webview: projection store, screens, design system | **Not started** — target architecture in `src/CONTEXT.md`; the core bridge it consumes is live |
...
The **Rust core (`src-tauri`) is built**; the **webview (`src`) module breakdown is target architecture** to build toward.
